...
//	protected bool avatarPosOverlaysBackground = true;
    ...
    private int[] boneIdsToTrack = { };
    public string GetJointPositions()
    {
        StringBuilder str = new StringBuilder();
        for(int i=0;i<bones.Length;i++)        {
            try
            {
                Vector3 trans = (GetBoneTransform(i).position);                
                str.Append(trans.ToString());
            }
            catch
            {
                //str.Append("NOTTRACKED");
            }
            str.Append("\t");
        }
        return str.ToString();
    ...
    public string GetJointHeaders()
    {
        StringBuilder str = new StringBuilder();
        str.Append("Time\tScore\t");
        for(int i=0;i<bones.Length;i++){
           
            try
            {

                str.Append(boneIndex2MecanimMap[i].ToString());
            }
            catch { }
            str.Append("\t");
        }
        str.Append("Upper Lip Raiser\t");
        str.Append("Jaw Lowerer\t");
        str.Append("Lip Stretcher L\t");
        str.Append("Lip Stretcher R\t");
        str.Append("Brow Lowerer L\t");
        str.Append("Brow Lowerer R\t");
        str.Append("Lip Corner Depressor L\t");
        str.Append("Lip Corner Depressor R\t");
        str.Append("Eyelid Closed L\t");
        str.Append("Eyelid Closed R\t");
        return str.ToString();
    }
